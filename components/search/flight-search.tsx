...
export function FlightSearch({ defaultOrigin = null, defaultDestination = null, compactTitle }: Props) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!origin?.code || !dest?.code) {
      toast?.error?.('Укажите город вылета и прилёта')
      return
    }
    setLoading(true)
    try {
  ...
  return (
    <form onSubmit={onSubmit} className="w-full rounded-2xl bg-white p-4 shadow-xl ring-1 ring-slate-100 md:p-5">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto_1fr_auto_auto_auto_auto]">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-500">Туда</label>
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-500">Обратно</label>
          <div className="relative">
            <CalendarDays className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
            <input
              type="date"
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-500">Пассажиры</label>
          <div className="relative">
            <Users className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
            <select
              value={String(adults ?? 1)}
              onChange={(e) => setAdults(parseInt(e?.target?.value ?? '1', 10) || 1)}
